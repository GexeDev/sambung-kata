...
controlPanel.Size = UDim2.new(0, 340, 0, 40)  -- lebih panjang buat 6 tombol
controlPanel.Position = UDim2.new(0.5, -170, 0.1, 0)
...
local colorBtn = makeButton(40, "🎨", Color3.fromRGB(255, 255, 255), controlPanel)
local upBtn = makeButton(75, "⬆️", Color3.fromRGB(0, 255, 0), controlPanel)
local downBtn = makeButton(110, "⬇️", Color3.fromRGB(255, 0, 0), controlPanel)
local rightBtn = makeButton(145, "➡️", Color3.fromRGB(0, 255, 0), controlPanel)
local leftBtn = makeButton(180, "⬅️", Color3.fromRGB(255, 0, 0), controlPanel)
local saveBtn = makeButton(215, "💾", Color3.fromRGB(0, 150, 255), controlPanel)
local resetBtn = makeButton(250, "↺", Color3.fromRGB(255, 255, 255), controlPanel)
local closeBtn = makeButton(285, "X", Color3.fromRGB(255, 100, 100), controlPanel)

-- =========================
-- TOMBOL ENTER (BISA DRAG)
-- =========================
...
local defaultPos = UDim2.new(1, -170, 0.02, 0)
local defaultSize = UDim2.new(0, 150, 0, 70)
...
enterFrame.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        if not locked then
            ...
            enterFrame.BackgroundTransparency = colorMode and 0.9 or 0.3
        end
...
enterFrame.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        dragging = false
        dragInput = nil
        enterFrame.BackgroundTransparency = colorMode and 0.8 or 0.1
        
        -- KIRIM KATA!
        if SubmitWord and matchActive and isMyTurn and serverLetter ~= "" and kataTerpilih ~= "" then
...
colorBtn.MouseButton1Click:Connect(function()
    ...
    if colorMode then
        enterFrame.BackgroundTransparency = 0.8
        enterStroke.Transparency = 0.8
...
-- RESIZE UP (TAMBAH TINGGI)
upBtn.MouseButton1Click:Connect(function()
    local currentSize = enterFrame.Size
    local newHeight = math.min(currentSize.Y.Offset + 10, 200)
...
downBtn.MouseButton1Click:Connect(function()
    local currentSize = enterFrame.Size
    local newHeight = math.max(currentSize.Y.Offset - 10, 40)
    enterFrame.Size = UDim2.new(0, currentSize.X.Offset, 0, newHeight)
    enterText.TextSize = math.floor(newHeight * 0.34)
    miniPreview.TextSize = math.floor(newHeight * 0.17)
end)

-- RESIZE RIGHT (TAMBAH LEBAR)
rightBtn.MouseButton1Click:Connect(function()
    local currentSize = enterFrame.Size
    local newWidth = math.min(currentSize.X.Offset + 20, 400)
...
leftBtn.MouseButton1Click:Connect(function()
    local currentSize = enterFrame.Size
    local newWidth = math.max(currentSize.X.Offset - 20, 80)
    enterFrame.Size = UDim2.new(0, newWidth, 0, currentSize.Y.Offset)
end)
...
-- INIT
-- =========================
print("🚀 ENTER BOT - 4 ARAH RESIZE + SAVE!")
print("📌 ⬆️⬇️ : atur TINGGI")
print("📌 ➡️⬅️ : atur LEBAR")
print("📌 💾 : simpan posisi & ukuran")
print("📌 🔓/🔒 : unlock/lock drag")
print("📌 🎨 : ganti warna transparant/abu")